                            ...
                            <?php if(isset($data) && !empty($data)):?>
                                <?php foreach($data as $key => $item):?>
                                                        ...
                                                        if($diff1 == $reminder_day){
                                                            ...
                                                            if($count <= $days_count){
                                                                if($item->reminder_email_sent == 0){
                                                                    ...
                                                                    $subject = str_replace("{support_request_subject}","Demande de Devis Handi-Express.fr",$subject);	
                                                                    $message = str_replace("Reply : {last_support_request_user_reply}","",$message);	
                                                                    $message = str_replace("{support_request_sender_email}",$item->email,$message);
                                                                    $message = str_replace("{support_request_date}",from_unix_date($item->created_at),$message);
                                                                    $message = str_replace("{support_request_time}",from_unix_time($item->created_at),$message);
                                                                    $message = str_replace("{support_request_civility}",$item->civility,$message);
                                                                    $message = str_replace("{support_request_first_name}",$item->first_name,$message);
                                                                    $message = str_replace("{support_request_last_name}",$item->last_name,$message);
                                                                    $message = str_replace("{support_request_company_name}",$item->company,$message);
                                                                    $message = str_replace("Subject : {support_request_subject}","",$message);
                                                                    $message = str_replace("{support_request_message}",$item->message,$message);
                                                                    if($item->status == "New"){
                                    ...
                                    <tr <?php if($item->unread == 1){ ?> class="unread" <?php }?>>
                                        ...
                                        <td>
                                            <a href="<?=site_url("user/support/".$item->id."/ticket_edit")?>">
