...
class Request_Model extends CI_Model {
	...
	public function getRequestReplyUser($id, $addedBy) {
        $dbQuery =  $this->db->query("SELECT u.username, d.name as department FROM vbs_request_replies as r left outer join vbs_users as u on u.id = r.addedBy left outer join vbs_departments as d on d.id = u.department_id WHERE r.id = ".$id." AND r.addedBy = ".$addedBy." ");
        return $dbQuery->result();
    }
    
    public function get_reply($request_id) {
        $dbQuery =  $this->db->query("SELECT * FROM vbs_request_replies rr WHERE rr.request_id = ".$request_id." AND rr.type = 2 order by id desc limit 1");
        return $dbQuery->result();
	}
